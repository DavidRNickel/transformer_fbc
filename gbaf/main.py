...
def test_model(test_bits, model, conf):
    model.eval()
    ...
    with torch.no_grad():
        for i in range(num_iters):
            bits = test_bits[batch_size*i : batch_size*(i+1)].to(device)
...
if __name__=='__main__':
    ...
    bs = conf.batch_size
    bitstreams_train = torch.randint(0,2,(conf.num_training_samps, conf.K))
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(conf.num_iters_per_epoch):
            ...
            if i % 25 == 0:
                ber, bler, _ = test_model(test_bits=torch.randint(0,2,(100000, conf.K)), model=fbc, conf=conf)
